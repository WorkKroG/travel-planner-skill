...
The HTML is derived and opens locally as a self-contained file. It uses one readable column, one compact sequence of overnight bases with dates/night counts, a closed native contents disclosure with date-and-place links, typed chronological day timelines, event-owned links and alternatives, and complete alternative scenario timelines. Do not repeat the route on the cover or add a separate list of day summaries or day filters. Keep introductions, travel/load facts and open questions with their existing owners; the cover's primary link goes directly to detailed days. Use the shared Lemon and Cobalt visual language: yellow cover/day openings, sans-serif place headings, gallery under the day introduction and above compact facts, cobalt controls, semantic icons on a continuous timeline and blue closing fields. Do not infer day phases from ambiguous labels, add full-text search or depend on host icon globals. JavaScript enhances scenario tabs and local alternatives; without it all content remains readable. If the user wants a PDF, browser Print → Save as PDF is a manual browser action, not an automated product artifact or guarantee.
