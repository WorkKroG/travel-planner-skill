...
**Текущий этап:** PR 4 находится в review; shared HTML и optional PDF проверяются до merge
...
| 3. Internal helpers and CLI | Превратить challenge в явные hard checks, сузить evidence, оставить `init/check/render/pdf`, удалить route/impact/migration/partial rebuild | Завершён в PR 3; squash-merged как `91773e1` |
| 4. Shared HTML and optional PDF | Свести поверхности к одному view model/template, реализовать метки Final и видимые принятые блокеры, удалить Markdown/receipt CLI-era complexity, оставить optional Python PDF; согласовать HTML spec | В review; merge gate остаётся закрытым до проверки PR |
| 5. Skill workflow | Привести SKILL.md и references к canonical bundle, четырём CLI-командам и lifecycle contract | Следующий PR после merge PR 4; текущие skill instructions пока сохранены |
...
2. Node/package/axe/Node Playwright и часть прежней UI QA-матрицы сохранены переходно до PR 6; они не определяют новый offline workflow.
...
Следующий gate — review и merge PR 4. После него PR 5 приводит SKILL.md и references к canonical bundle, четырём CLI-командам и lifecycle contract. Eval reduction и packaging остаются PR 6–7.
