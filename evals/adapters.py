...
class FixtureAdapter:
    ...
    def run(self, prompt: str, workspace: Path) -> AgentRun:
        ...
        reference_evaluator = scenario.get("reference_evaluator")
        if isinstance(reference_evaluator, Mapping):
            from .reference_evaluator import evaluate

            contract = {
                "version": reference_evaluator.get("version"),
                "kind": reference_evaluator.get("kind"),
                "parameters": reference_evaluator.get("parameters"),
            }
            return evaluate(scenario_id, reference_evaluator["fixture_input"], contract)
