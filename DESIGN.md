...
This document records the implemented shared template in `skills/travel-planner/assets/html/styles.css` and `itinerary.html.j2`, following the [approved September 7 visual specification](docs/superpowers/specs/2026-09-07-html-visual-redesign-design.md) and the one-day Hakone composition approved for the full template on September 8. That composition governs the three-fact header, larger circular markers and recorded day periods. Later 8 September refinements remove research lists from the reader HTML and add visible situation/safety and visa/entry/transit reviews in Preparation. Practical links stay beside events and preparation actions, with compact photo credits. It describes source-level properties; it does not record a completed browser review.

Preparation uses the existing divided readiness list. Review items show their result, actual review date or “Not checked”, next action, a bold “Recheck required” label and contextual links using the existing external-link treatment. The text remains visible without JavaScript, without a disclosure, and in print, including for completed reviews. No new card system, source index or automatic freshness indicator is introduced.
