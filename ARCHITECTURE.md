...
**Статус:** утверждённая целевая граница; переход выполняется последовательностью PR

**Область ответственности:** минимальные компоненты и их обязанности

Архитектура сохраняет сложность, непосредственно защищающую пользователя сложной поездки, и удаляет инфраструктуру, которая пока не подтверждена внешним использованием. Текущий код ещё не полностью соответствует этой схеме; фактический прогресс отражён в [docs/PROJECT_STATUS.md](docs/PROJECT_STATUS.md).
...
До завершения этих PR существование старого модуля или теста описывает текущую реализацию, но не расширяет продуктовый контракт.
