...
| Sources report | Текущий `sources.md` и неизменяемый перечень материалов каждой HTML-сборки | Универсальный evidence engine, восстановление старого состояния без сохранённого перечня |
...
- `render` — сохранить технический перечень источников версии и построить общий self-contained HTML.

CLI сначала полностью рендерит документ и вычисляет SHA-256 его UTF-8 bytes. `evidence.write_source_snapshot` сохраняет `sources/<SHA256>.md` относительно каталога выбранного HTML: время сборки, lifecycle, хеши пяти канонических файлов, полный реестр источников, claims, readiness, фотографии и ссылки событий. Полностью записанный временный файл публикуется без замены существующего перечня; одинаковое содержимое переиспользуется, конфликт требует нового `--at`. Лишь после этого HTML атомарно заменяет предыдущий файл. Ошибка сохранения перечня не публикует новый HTML. При ошибке последующей записи HTML может остаться неиспользованный технический перечень; прежний HTML сохраняется. Генерация не изменяет канонические файлы или текущий `sources.md`.

Запрос через Codex использует SHA-256 предоставленного HTML для выбора сохранённого перечня. Отсутствующий исторический файл нельзя заменить текущими источниками. Перечни — локальные производные артефакты, которые сохраняют между сборками; это не backend, sync или архив всех версий канонического состояния. Низкоуровневый `render_html` возвращает только HTML; полный workflow обязан сохранить перечень, как это делает CLI.
...
Хеши канонических файлов фиксируются до загрузки состояния и повторно сверяются перед сохранением перечня. Изменение входов во время сборки останавливает публикацию. Metadata в перечне сериализуется штатным YAML dumper-ом с сохранением смешанных типов ключей и точных Decimal-значений.
