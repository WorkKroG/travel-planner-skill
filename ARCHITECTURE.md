...
Выбор статуса принадлежит пользователю и применяется workflow модели: новая поездка и промежуточный HTML остаются `draft`, а оба основания `final` требуют явного решения пользователя о финализации этой версии. Решение и его область сохраняются в существующий `decisions.md`; нового поля согласия или команды финализации нет. `check` и `render` не читают разговор для вывода согласия и не меняют статус. При существенной переработке финального плана модель согласует предложенный возврат в черновик вместе с изменением; выбранный пользователем возврат очищает `finalization_basis`. Опечатки и оформление сохраняют статус. Схемы и проверка согласованности полей остаются прежними; существующие финальные записи не понижаются автоматически при открытии.
